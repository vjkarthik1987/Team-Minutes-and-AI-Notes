   ...
   */
  function transcriptTimeCandidates(t) {
    const values = [
      t?.startDateTime,
      t?.endDateTime,
      t?.createdDateTime,
      t?.meetingStartDateTime,
      t?.meetingEndDateTime,
    ];
    return values.map(toMs).filter(Number.isFinite);
  }

  function occurrenceWindowForEvent(ev, opts = {}) {
    const startMs = toMs(ev?.start?.dateTime || ev?.startDateTime || ev?.start);
    const endMs = toMs(ev?.end?.dateTime || ev?.endDateTime || ev?.end);
    const beforeMs = Number(opts.beforeMs ?? 10 * 60 * 1000); // allow tiny clock skew only
    const afterMs = Number(opts.afterMs ?? 10 * 60 * 60 * 1000); // Teams transcript processing delay
    ...
    return {
      startMs: safeStart,
      endMs: safeEnd,
      windowStart: safeStart - beforeMs,
      windowEnd: safeEnd + afterMs,
      anchor: Number.isFinite(endMs) ? endMs : safeStart,
    };
  }

  function transcriptMatchesEventOccurrence(t, ev, opts = {}) {
    const w = occurrenceWindowForEvent(ev, opts);
    if (!w) return { ok: false, reason: 'no-event-time' };
    const times = transcriptTimeCandidates(t);
    if (!times.length) return { ok: false, reason: 'no-transcript-time' };
    const inWindow = times.some(x => x >= w.windowStart && x <= w.windowEnd);
    if (!inWindow) return { ok: false, reason: 'outside-occurrence-window' };
    const bestDist = Math.min(...times.map(x => Math.abs(x - w.anchor)));
    return { ok: true, dist: bestDist, reason: 'matched-occurrence-window' };
  }

  /**
   * Pick the transcript that best matches THIS calendar occurrence.
   * Same Teams link can have many transcript instances. We must match by occurrence time,
   * not by link alone. If nothing fits this event time window, return null.
   */
  function pickBestTranscriptForEvent(items, ev) {
    if (!Array.isArray(items) || items.length === 0) return null;

    const w = occurrenceWindowForEvent(ev);
    if (!w) return null;

    const scored = [];
    for (const t of items) {
      const match = transcriptMatchesEventOccurrence(t, ev);
      if (!match.ok) continue;
      scored.push({ t, dist: match.dist ?? Number.POSITIVE_INFINITY });
    }

    scored.sort((a, b) => a.dist - b.dist);
    const best = scored[0]?.t || null;
    if (best) {
      best._msMinutesOccurrenceMatched = true;
      best._msMinutesOccurrenceReason = 'time-window';
    }
    return best;
  }
    ...
    await runWithLimit(candidates, concurrency, async (ev) => {
      try {
        ...
        ev._transcripts = [{
          ...
          startDateTime: best.startDateTime || null,
          endDateTime: best.endDateTime || null,
          meetingId: mtg.id,
        }];
  ...
  module.exports = { annotateEventsWithTranscripts, getTranscript, listTranscripts, pickBestTranscriptForEvent, transcriptMatchesEventOccurrence };
